...
document.addEventListener("alpine:init", () => {
  ...
  Alpine.data("distritosChartComponent", () => ({
    ...
    fetchData(id, inicio, fin) {
      ...
      const params = new URLSearchParams({ IdDiagnostico: id, FechaInicio: inicio, FechaFin: fin });
      fetch(`/api/distritos-por-diagnostico?${params}`).then((res) => res.json()).then((data) => {
        if (data && data.length > 0) {
  ...
  Alpine.data("tiempoChartComponent", () => ({
      ...
      const options = {
        ...
        tooltip: {
          custom: function({ series, seriesIndex, dataPointIndex, w }) {
            ...
            return '<div class="px-3 py-2 bg-white border border-slate-200 rounded shadow-lg">' + '<div class="text-xs text-slate-500">' + fecha + "</div>" + '<div class="text-sm font-semibold text-slate-800">Atenciones: ' + value + "</div>" + "</div>";
          }
        },
        noData: { text: "Seleccione un diagnóstico y rango de fechas." }
      };
    ...
    fetchData(id, inicio, fin) {
      ...
      const params = new URLSearchParams({ IdDiagnostico: id, FechaInicio: inicio, FechaFin: fin });
      fetch(`/api/atenciones-por-dia?${params}`).then((res) => res.json()).then((data) => {
        if (data && data.length > 0) {
